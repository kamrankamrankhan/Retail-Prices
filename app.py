...
<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 1.5rem; border-radius: 10px; margin: 2rem 0;">
    <h2 style="color: white; text-align: center; margin: 0;">🎯 Real-Time Price Prediction Calculator</h2>
    <p style="color: white; text-align: center; margin: 0.5rem 0 0 0; opacity: 0.9;">Input product parameters to get instant price predictions</p>
</div>
""", unsafe_allow_html=True)
...
with col1:
    ...
    # Real-time inputs (no form wrapper for instant updates)
    selected_category = st.selectbox(
        "🏷️ Product Category",
        options=product_categories,
        help="Select the product category",
        key="realtime_category"
    )
...
with col2:
    ...
    # Real-time prediction logic (runs on every input change)
    ...
    # Real-time insights and warnings
    st.markdown("### 💡 Smart Insights")
    
    # Price positioning analysis
    market_position = ""
    if unit_price > competitor_price * 1.1:
        market_position = "🔴 Premium Position"
        st.warning("⚠️ Your price is significantly higher than competitors. Consider value proposition.")
    elif unit_price < competitor_price * 0.9:
        market_position = "🟢 Budget Position"
        st.success("✅ You're positioned as a budget option. Good for market penetration.")
    else:
        market_position = "🟡 Competitive Position"
        st.info("ℹ️ You're competitively priced. Monitor market changes.")
    
    # Profit margin analysis
    if profit_margin > 20:
        st.success("💰 Excellent profit margin! Your pricing strategy is working well.")
    elif profit_margin < 5:
        st.error("⚠️ Low profit margin. Consider cost optimization or price adjustment.")
    else:
        st.info("📊 Moderate profit margin. Room for optimization.")
    
    # Quantity vs price analysis
    if quantity > 500 and unit_price < 30:
        st.success("📦 High volume, low price strategy detected. Good for market share.")
    elif quantity < 50 and unit_price > 100:
        st.info("💎 Low volume, high price strategy. Focus on premium positioning.")
